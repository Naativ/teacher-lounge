  ...
  <div id="vipRoom">
    ...
    <div v-if="!showConnect && !connected">
      <v-layout
        ...
        align-center
      >
        <h5>Your conversation starts:</h5>
          <h5 style="padding-top: 20px;" v-if="booking">{{timeTo}}</h5>
          <v-layout style="padding-top: 20px;" row wrap justify-space-around class="connect-room">
            <v-btn
              @click="connectRoom"
              :loading="connecting"
    ...
    <v-layout v-if="sessionOver && !isHost">
    </v-layout>
    <p v-if="remoteConnected " style="margin: 0; padding: 0;">{{ remoteName }}</p>
    <v-snackbar class="tip" v-model="tip" top :timeout="5000">
      For a better experience, make sure you are connected to WiFi before connecting to the call!
    </v-snackbar>
    <v-card id="vipContainer">
      ...
      <div class="video-wrapper">
        <div v-if="!sessionOver" class="av-wrapper">
        ...
        <v-flex xs12>
          <v-tooltip left>
            <template slot="activator">
          <v-btn color="rgb(0,0,0,.4)" icon @click="confirmLeave">
...
import { MEMBER_INFO } from '@/graphql/Member.gql.js'
...
import EndConversation from '@/components/classroom/EndConversation.vue'
...
export default {
  ...
  methods: {
    ...
    participantDisconnected(participant) {
      const matchingEmail = this.$store.state.member.contactEmail === participant.identity
  ...
  computed: {
    ...
    timeTo() {
      return this.$moment().to(this.booking.start)
    }
  }
}
</script>
